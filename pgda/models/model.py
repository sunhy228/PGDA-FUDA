...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_feature):
        ...
        out8 = torch.flatten(out7, 1)
        out9 = self.fc(out8)
        if return_feature:
            return out5, out9
        elif self.embed_only:
            return out8
        else:
            return out9
...
class ResNet18(nn.Module):
    def __init__(self, num_classes=10,  embed_only=False, from_scratch=False, path_to_weights="./test/temp/resnet18-5c106cde.pth", device="cuda"):
    ...
    def forward(self, x, return_feature=False):
        return self.resnet18(x, return_feature)
...
class LinearClassifier(nn.Module):
    def __init__(self, name='resnet18', num_classes=6, device="cuda"):
